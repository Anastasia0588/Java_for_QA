...
public class JamesHelper {
    ...
    public void createUser(String name, String passwd) {
        ...
        String result = readUntil("User " + name + " added");    // ждем, пока на консоль появится такой текст
    ...
    private void initTelnetSession() {
        ...
        try {
            ...
            in = telnet.getInputStream();    // берем входной поток, чтобы что-то читать - те данные, которые TelnetClient отправляет нам
            out = new PrintStream( telnet.getOutputStream() );    // берем выходной поток, чтобы что-то писать - отправлять команды TelnetClient
    ...
    public void drainEmail(String username, String password) throws MessagingException { // удлаяет все письма, полученные определенным пользователем (очищает его почтовый ящик)
    ...
    private Folder openInbox(String username, String password) throws MessagingException {  // открываем почтовый ящик
        ...
        Folder folder = store.getDefaultFolder().getFolder("INBOX");  // получаем доступ к папке INBOX, стандартное название папки, по протоколу POP3 можно получить доступ только к ней
    ...
    public List<MailMessage> waitForMail(String username, String password, long timeout) throws MessagingException {
        ...
        while (System.currentTimeMillis() < now + timeout) { //проверяется, что текущее время не превышает момента старта плюс заданный таймаут
